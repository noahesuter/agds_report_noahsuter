...
We can see that some columns do have a lot of missing values. It will be important to keep that in mind for the interpretation of the results and for the creation of the model. For creating a model i chose to delete the rows containing missin values.
...
One can see the bivariate models already sorted after the value AIC. The sorting is descending from the lowest to the highest AIC. As the lowest AIC is the best, this predictor should be used. In this case it would be PPFD_IN. In the case that the option <none> would be better than all the predictors, the model would be better without any predictors. But that is not the case here. One can see that the R squared as well as the adjusted R squared is decreasing if higher up. That makes sense as the model is better if there is a higher R squared. The higher the adjusted R squared, the higher the Generalizability. Therefore it makes sense that the best model (highest AIC) has the highest adjusted R squared as well. And the highest R squared. For the stepwise forward regression, the model should continue with PPFD_IN set as the first predictor.
...
```
The choice of the model makes sense as the lowest AIC is being chosen. AS a lower AIC means that the model is better, the result makes sense. It as well makes sense that it stopped after the predictor P_F. Because the model had better AIC values (lower AIC) if no more predictors were added. The adjusted R squared got better as well which means that it got higher. The R squared is not the highest within the last model but that makes sense at the r squared is not the most important value to determine which model is the best (AIC is better)
```{r warning=FALSE}
library(tidyverse)
library(psych)
