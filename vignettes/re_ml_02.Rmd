...
```{r}
model_dav_train <- recipes::recipe(GPP_NT_VUT_REF ~ SW_IN_F + VPD_F + PA_F + P_F + WS_F + TA_F, 
...
### Using the caret function to determine the best model with maschine learning
```{r}
library(tidyverse)
library(recipes)
set.seed(1982)
...
```{r}
ggplot(caret_dav_train_knn)
```

```{r}
print(caret_dav_train_knn)
```
