...
```{r}
test_model <- lm(GPP_NT_VUT_REF ~ TIMESTAMP, data = half_hourly_fluxes_clean)
plot(test_model, 1)
```
...
```
afterwards one could run the stepwise forward regression with the imputed data and then compare the outcome with the model based on the deleted missing values. Then one could check which model is better and makes more sense. But as the mice function would need too long to knit i won't to the stepwise forward regression.
