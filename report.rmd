...
### Task 1
```{r}
list.files('C:/Users/noahe/OneDrive/Dokumente/Studium/Studium 4. Semester/AGDS/Data') #listing the files to find the right data
```
```{r}
database_s1<- readr::read_csv("C:/Users/noahe/OneDrive/Dokumente/Studium/Studium 4. Semester/AGDS/Data/data_csv_exercise_1.csv") # reading the .csv into R
```
...
sample_date_yearly <-  sample_data_RR|> #create new table
  ...
  dplyr::summarise(`log_response_ratio_mean` = mean(`log_response_ratio`)) #calculating mean within the defined groups of the values co2 (ambient and increased)
```
...
phase_1 <- dplyr::filter(sample_data_RR, `Time (years)` <3) |>
  dplyr::summarise(`log_response_ratio_mean` = mean(`log_response_ratio`)) #aggregate phase 1
```
...
phase_3 <- dplyr::filter(sample_data_RR, `Time (years)` >=6) |>
  dplyr::summarise(`log_response_ratio_mean` = mean(`log_response_ratio`)) #aggregate phase 3
```
...
```{r}
all_phases <- dplyr::select(all_phases,`Phase`, everything()) #placing the phase column to position one
```
